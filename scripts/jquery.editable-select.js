...
(function( $ ){
  ...
  var methods = {
     ...
     init : function( options ) {
		...
		return this.each(function() { 
			...
			if (settings.allowNewElements){
				selectElement.prepend('<option>' + newValueString + '</option>');
			}
			...
			selectElement.keydown( function(e){
				...
				if(e.keyCode == "46"){ // del-button
					if(selected != newValueString){
						$(this).children("option:selected").remove();
					}
					return;
				}
				
				if( selected==newValueString || settings['editableElements'] ) {
	 ...
	 enableAddingNewElements: function(){
		var fChild = $(this).children().first();
		if(fChild.text() != newValueString){
			$(this).prepend('<option>' + newValueString + '</option>');
		}
	 },
